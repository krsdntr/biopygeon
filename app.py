...
header[data-testid="stHeader"]::after {
    content: "🕊️ Biopygeon AI Assistant";
    position: absolute;
    left: 3.5rem;
    top: 50%;
...
# --- SETTINGS MODAL ---
@st.dialog("⚙️ Pengaturan Biopygeon")
def settings_modal():
    ...
    if groq_key != initial_groq_key:
        ...
        st.toast("Kunci API Groq berhasil diperbarui!", icon="🔑")
    ...
    if user_email != initial_email:
        ...
        st.toast("Email pengguna berhasil diperbarui!", icon="✉️")
    ...
    if s2_key != initial_s2_key:
        set_s2_key(s2_key)
        st.toast("Kunci API Semantic Scholar berhasil diperbarui!", icon="🔑")
...
with st.sidebar:
    ...
    with st.container():
        ...
        if history_dict:
            if st.button("🗑️ Kelola Riwayat", use_container_width=True):
                manage_chats_modal()
    ...
    with st.container():
        if not history_dict:
            ...
            for sid, sdata in sorted_history[:st.session_state.history_limit]:
                ...
                btn_label = f"{prefix} {sdata['title']}"
...
if len(st.session_state.messages) == 0:
    # Header Section
    st.markdown("<h1 style='margin-bottom: 5px;'>🕊️ Biopygeon AI Assistant</h1>", unsafe_allow_html=True)
    ...
    st.markdown("##### 💡 Mulai dengan salah satu saran berikut:")
    ...
    with st.expander("✨ Lihat Katalog Kemampuan Biopygeon"):
        features_html = '''
...
if chat_submission:
    ...
    if uploaded_files:
        for uf in uploaded_files:
            file_path = os.path.join(os.getcwd(), uf.name)
            # Write bytes to disk
            ...
            if uf.name not in st.session_state.uploaded_files:
                ...
                st.toast(f"Berhasil mengunggah: {uf.name}", icon="📁")
    ...
    with st.chat_message("assistant"):
        ...
        try:
            ...
            # 6. DOWNLOAD PROTEIN FILES
                ...
                downloaded = download_protein_files(pdb_id, file_type, os.getcwd(), progress_callback=ui_progress_callback)
                if not downloaded:
                    status_box.update(label="Gagal mengunduh struktur", state="error")
                else:
                    msg_text = f"{reply}\n\n**File struktur berhasil diunduh ke workspace:**\n"
                    for f in downloaded:
                        msg_text += f"- `{os.path.basename(f)}`\n"
            ...
            # 13. PLOT PHYLOGENETIC TREE
                ...
                if os.path.exists(res_path):
                    ...
                    msg_downloads.append({
                        "label": "🖼️ Unduh Publikasi TIFF (300 DPI)",
                        "data": f_data,
                        "file_name": os.path.basename(res_path),
                        "mime": "image/tiff"
                    })
            ...
            # 16. RENDER SSN NETWORK
                ...
                if os.path.exists(out_tiff):
                    ...
                    with open(out_tiff, "rb") as f:
                        t_data = f.read()
                    msg_downloads.append({
                        "label": "🖼️ Unduh Jaringan TIFF (300 DPI)",
                        "data": t_data,
